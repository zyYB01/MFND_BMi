...
class KalmanDecoder:
    ...
    def predict(self, X_test, y_init):
        ...
        n_states = X.shape[0]
        states = np.empty((n_states, Z.shape[1]))
...
class MultiClassReFITKalmanDecoder:
    def __init__(self, reg_alpha=0, n_classes=3):
        self.reg_alpha = reg_alpha
        self.n_classes = n_classes

    def fit(self, X_train, y_train):
        regres = LogisticRegression(penalty='none', solver='saga', multi_class='multinomial', alpha=self.reg_alpha)
...
def QRNNDecoder_BH(config):
   
    model = Sequential()
    for i in range(config["n_layers"]):
        if i == config["n_layers"] - 1:
            model.add(QRNN(config["units"], input_shape=(config["timesteps"], config["input_dim"]),
                           window_size=config["window_size"], return_sequences=False))
        else:
            model.add(QRNN(config["units"], input_shape=(config["timesteps"], config["input_dim"]),
                           window_size=config["window_size"], return_sequences=True))
    if config["dropout"] > 0:
        model.add(Dropout(config["dropout"]))
    model.add(Dense(units=3, activation='softmax'))  # Changed output_dim to 3 and added softmax activation
    ...
    model.compile(loss='categorical_crossentropy',  # Changed loss function to categorical_crossentropy
                  optimizer=opt,
                  metrics=[config["metric"]])  # Using the specified metric
